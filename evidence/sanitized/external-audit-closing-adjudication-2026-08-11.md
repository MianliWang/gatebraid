...
standing discipline, per templates/consult.md).
...
| B2 | P1-1 adopted with no phase delivering `gate-run@2`; new M3 gate records would have no valid schema | **ACCEPT (placement adapted)** | M3-PLAN §2 as committed names no `@2` delivery home — confirmed. In fairness: N1–N3 accept via batch readbacks, not gate exits, so the audit's "N1/N2/N3 will generate gate records" overstates; the first M3 gate exit is O1's. The gap stands regardless | `@2` admitted at N1 as schema-with-fixtures; O1 writes the first M3 gate record as `@2`; no bootstrap exception needed (M3-PLAN §2 N1; ADR-0029 P1-1 bullet) |
