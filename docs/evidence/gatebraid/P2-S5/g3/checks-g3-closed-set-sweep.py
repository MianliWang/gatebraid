...
GATE 3 COPY. This file is a copy of the Gate 2 instrument
docs/evidence/gatebraid/P2-S5/g2/checks-g2-closed-set-sweep.py (sha256
e99b236d27cea0b79e9da29cd82f557c39efebeda3ce6ae82d8f70ec300781f0), which Gate 2's
captures pin by hash and which therefore rides onto the branch byte-identical and
is NOT edited. That copy is itself a copy of the Gate 1 copy, which is a copy of
the retained Gate 0 original; the whole chain is unedited behind this file. The
Gate 0, Gate 0 re-run, Gate 1 and Gate 2 copies are all untouched by this gate.

THIS HEADER DESCRIBES ONLY WHAT THIS FILE'S BYTES CARRY. Every item below is
present in the code, and nothing present in the code is omitted below.

FOUR groups differ from the Gate 2 copy, all of them DOMAIN FACTS, none of them a
rule. No classification rule, no regex and no residue criterion changed;
`classify_repo` is untouched and the criterion is still
`sys.exit(1 if residue else 0)`. The permitted set, the permitted Project, the
permitted item namespace, the subject issue and the mention class are all
byte-identical to the Gate 2 copy.

  G3-a. CAPDIR's default names this gate's captures directory, and the
        self-exclusion prefix is G3-, matching this gate's capture ids.
  G3-b. FS_PREFIX gains "actions" and "origin", TRANSCRIBED VERBATIM from the
  ...
  G3-d. The mention class is UNCHANGED from the Gate 2 copy. This gate's own
        ...
        class was widened to accommodate it, and a bare `#21` written anywhere
        would still be residue.

THE DELIBERATE RESIDUE THE GATE 2 COPY DISCLOSES IS NOT PRESENT HERE, and that
is a fact about the domain rather than about the rules. That copy's header
records one residue left unexplained on purpose: a frozen-corpus case label
carrying an issue-shaped token that is a friction citation written without the
word `friction`. This gate ran no corpus, so no such label is in its domain.
Residue over this gate's own captures is EMPTY, and it is empty without any rule
having been loosened to make it so.

FALSIFIED BEFORE TRUST, in THREE seeded runs, all captured, all of them run
before the real one.

  1. The RETAINED Gate 1 seeds, which must still fire the repository, node and
     issue limbs through this copy. They do: 5 residues across all three limbs.
  2. The Gate 2 near-miss seed, re-run here, so the fourteen exact-string facts
     that copy added are shown still exact through this one and not widened by
     anything changed here. All 15 of its tokens remain residue.
  3. A NEW seed for every fact added above: a token shaped like it but outside
     it, one character different. All 12 remain residue. A fact that admitted
     its own near-miss would be a blindfold rather than a domain fact.

Seed 3 carries two tokens for a second reason, and they test this header's own
claims rather than the classifier's. `remotes/tags` is seeded where no slash
precedes it, because the G3-c note above claims the extractor's lookbehind stops
after the first pair so that this gate's drift label yields `heads/remotes`
ALONE; were that claim wrong, the label would also have produced remotes/tags
and admitting one token would have left a second silently unexplained. And `#21`
is this gate's own pull request in the one shape the issue extractor accepts,
seeded because G3-d claims the mention class was NOT widened for it. Both remain
residue, and so does `heads/remotes.` with its trailing period, which is what an
exact string does and a prefix rule does not.
"""
...
CAPDIR = sys.argv[1] if len(sys.argv) > 1 else "docs/evidence/gatebraid/P2-S5/g3/captures"
...
FS_PREFIX = {
    ...
    "g2",         # this Slice's Gate 2 evidence subdirectory
    # --- new in the Gate 3 copy, with stated reasons.
    "g3",         # and its Gate 3 one, this gate's own
    "actions",    # the GitHub REST path fragment actions/workflows, read by
                  # the CI row. Transcribed from the merged P2-S6 Gate 3 copy
                  # together with its own stated reason.
    "origin",     # `git push -u origin slice/P2-S5` splits to origin/slice:
                  # a remote name joined to a branch name, not an owner.
...
PROSE_PAIRS = {
    ...
    "heads/remotes",      # this gate's drift row prints the label "refs outside
                          ...
                          # slash-joined in a human-readable column heading. The
                          # extractor's lookbehind stops after the first pair,
                          # so the label yields this one token and not also
                          # remotes/tags. It is prose, not an owner.
...
SELF = [f for f in ALL if os.path.basename(f).startswith("G3-closed-set-sweep")]
