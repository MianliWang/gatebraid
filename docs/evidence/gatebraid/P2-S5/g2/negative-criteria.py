...
GATE 2 COPY. A copy of docs/evidence/gatebraid/P2-S5/g1/negative-criteria.py
(sha256 5014fec93f73b70d218e9f783b20f3034dd45a207185e1b0b181bb133d773621), which Gate 1's captures pin by hash and
which therefore rides onto the branch byte-identical and is NOT edited.

ONE thing differs, and it is a DOMAIN FACT, not a rule:

  G2-a. N3's content limb excludes `g2` as well as `g0r` and `g1`.

The frozen plan states the property as "the file count of
docs/evidence/gatebraid/P2-S5/ WITH THE RE-RUN AND THIS GATE'S OWN
SUBDIRECTORIES EXCLUDED must be forty-three". At Gate 1 this gate's own
subdirectory was `g1` and `g2` did not exist, so the Gate 1 mechanisation named
two. At Gate 2 there are three, and naming only two makes the instrument count
THIS GATE'S OWN EVIDENCE as retained record and report a digest change that the
retained record did not undergo. The Gate 1 run is retained at
docs/evidence/gatebraid/P2-S5/g2/captures/G2-R-n3-g1-instrument-fired.json.

This copy LOOSENS NOTHING. The expected count is still 43, the expected digest
is still 83b3a273a9bd7da4e9e11469539a5eee0f28b53f5b924c0e6134acd8ba49a70f, and
both gate0.md hashes are unchanged; the same three constants are compared to the
same three measurements. The exclusion set is written as an explicit tuple of
names, never a pattern, so a fourth subdirectory would have to be added
deliberately and would be visible in a diff. Every other criterion, every
regex and every exit rule is byte-identical to the Gate 1 file.

Falsified before trust, like the Gate 1 file: the same seeded changed-path
list, code surface and frozen root, all six criteria required to fire.
...
def n3_content(root):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        # G2-a: this Slice's per-gate subdirectories, named explicitly.
        # The frozen plan says "the re-run and this gate's own
        # subdirectories"; at Gate 2 that is three names, not two.
        dirnames[:] = [d for d in dirnames
                       if d not in ("g0r", "g1", "g2")]
        for fn in filenames:
