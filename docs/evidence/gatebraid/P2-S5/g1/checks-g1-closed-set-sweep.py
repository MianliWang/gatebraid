...
GATE 1 COPY. This file is a copy of the Gate 0 re-run instrument
docs/evidence/gatebraid/P2-S5/g0r/checks-g0r-closed-set-sweep.py (sha256
d2b501555a223e5d69720fed3cf8640e56233d2f4d81549a87ca02788ad3bff1), which the
CLOSED Gate 0 record's captures pin by hash and which is therefore frozen. A
closed gate's instruments are not editable by a later gate, so the Gate 1
additions live here and the Gate 0 copies are left alone. THREE things differ
from that copy, all of them DOMAIN FACTS, none of them a rule:

  G1-a. CAPDIR's default names this gate's captures directory, and the
        self-exclusion prefix is G1-, matching this gate's capture ids.
  G1-b. The mention class gains 2, 3, 4 and 5 -- the sample Slices of the
        historical working repository. Gate 1 READS the M2 slice-C historical
        record to obtain the frozen scope, so those documents' own issue
        references necessarily enter this gate's domain. They are mention-class
        exactly as 14 and 16 were for the first attempt and 19 and 20 for the
        re-run: named in evidence, targeted by no query in this gate. The
        mention-class invocation check still applies to them, so a query that
        DID target one is still caught.
  G1-c. PROSE_PAIRS gains the pairs this gate's own plan prose contains, each
        named as an exact string and never as a pattern.

The Gate 0 re-run copy's own header follows, unchanged, because the six domain
facts it records are still in force here.
...
CAPDIR = sys.argv[1] if len(sys.argv) > 1 else "docs/evidence/gatebraid/P2-S5/g1/captures"
...
SELF = [f for f in ALL if os.path.basename(f).startswith("G1-closed-set-sweep")]
