...
CAPDIR = sys.argv[1] if len(sys.argv) > 1 else "docs/evidence/gatebraid/P2-S6/g3/captures"
...
FS_PREFIX = {
    ...
    "g2",         # and its Gate 2 one
    "g3",         # and its Gate 3 one
    "actions",    # the GitHub REST path fragment actions/workflows
    "origin",     # `git push -u origin slice/P2-S6` splits to origin/slice:
...
# Git branch refs this Slice legitimately names. An explicit set, never a
# pattern: the Slice branch namespace follows the four prior Slice branches
# (`slice/P1-S1`, `slice/P2-S1`, ...) and is a ref, not an owner-slash-repo.
BRANCH_REFS = {"slice/P2-S6"}

# A live snapshot carries the node id of EVERY row of the Project it read, not
# just this Slice's. Those are items of the permitted Project and the id says
# so: the Project id `PVT_kwHOBRofUs4Beum7` and every item id
# `PVTI_lAHOBRofUs4Beum7...` share this segment, measured across all sixteen
# rows of the captured read. Matching on it is a DISCRIMINATOR, not a blanket
# allowance - the falsification seed's `PVTI_ZZZSEEDNOTTHEITEM` does not carry
# it and still reaches the residue.
PROJECT_DISCRIMINATOR = "HOBRofUs4Beum7"

# JSON pointer and schema-path fragments printed by the corpus runner and by the
# independent validator, and two ordinary prose slashes. Every one is a DOCUMENT
# FIELD PATH, not an owner-slash-repo. Named as exact strings rather than matched
# by a shape: a regex over this shape is precisely the pass-1 defect the P2-S5
# sweep was repaired for, because it would swallow the very thing being hunted.
POINTER_SEGMENTS = {
    "streams/stdout", "items/required", "dual_platform_claim/reports",
    "self_assertions/zero_lone_cr", "invocation/shell_semantics",
    "generator/source_sha256", "invocation/cwd", "platform/os",
    "invocation/form", "invocation/environment",
    "self_assertions/binary_mode_write", "stop_record/remediation_attempted",
    "repair_attempts/0x1", "validator/source_sha256", "findings/0x1",
    "completeness/unexamined",
    "locus/loci",              # "0 error locus/loci", the validator's own prose
    "dependency/dependencies",  # "blocked by 1 open dependency/dependencies"
}

# Friction ordinals the corpus runner prints as a bare `#N` inside a case label,
# without the word `friction` the FRICTION rule keys on. Named explicitly.
BARE_FRICTION_REFS = {94}
...
def classify_repo(tok, where):
    ...
    if tok in BRANCH_REFS:
        return "E9 this Slice's own git branch ref (named, not matched)", None
    if tok in POINTER_SEGMENTS:
        return "E10 JSON pointer or schema-path fragment (named, not matched)", None
...
for f in files:
    ...
    for where, t in texts(f):
        ...
        for m in NODE.finditer(t):
            ...
            elif n.startswith("PVTI_") and PROJECT_DISCRIMINATOR in n:
                buckets["N4 item id of the permitted Project"] += 1
            else:
